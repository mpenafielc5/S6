...
class Product(models.Model):
    ...
    description = models.CharField('Articulo', max_length=100, unique=True)
    cost = models.DecimalField('Costo Producto', max_digits=10, decimal_places=2, default=0.0)
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    supplier = models.ForeignKey(Supplier, on_delete=models.CASCADE, verbose_name='Proveedor')  # Cambiado de OneToOneField a ForeignKey
    ...
    objects = models.Manager()  # Manager predeterminado
    active_products = ActiveProductManager()  # Manager filtrado por state=True
    ...
    def clean(self):
        if self.price < 0.01:
            raise ValidationError(_('El precio del producto no puede ser menor que 0.01.'))
